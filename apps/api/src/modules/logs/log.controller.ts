...
class LogController {
  async sendLogs(req: Request, res: Response) {
    ...
    const data = await logService.listSendLogs({ status, runId, cycleNumber });
    res.json(data);
  }
  ...
  async cycleDetail(req: Request, res: Response) {
    ...
    const data = await logService.getLogsForCycle(runId, cycle);
    res.json(data);
  }
  ...
  async exportSendLogs(req: Request, res: Response) {
    const runId = req.query.runId as string | undefined;
    const csv = await logService.exportSendLogsCsv(runId);
